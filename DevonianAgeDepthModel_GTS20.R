...
# Run ageModel.R code.
DevonianModel_Kaufmann <- ageModel(ages = DevonianData$age,
                          ageSds = DevonianData$error_2sig_nolambda,
                          positions = DevonianData$midpoint_Kaufmann,
                          positionThicknesses = DevonianData$halfwidth_Kaufmann,
                          distTypes = DevonianData$distTypes,
...
DevonianPredict_Kaufmann <- agePredict(model = DevonianModel_Kaufmann,
...
modelPlot(model = DevonianModel_Kaufmann,
...
modelPlot(model = DevonianModel_Kaufmann,
          agePredictOutput = DevonianPredict_Kaufmann,
          scale = 0.5, predictLabels = c("both"), legend = c("adjacent"))

# Visualize the parameter plots.
# posteriorPlot(model = DevonianModel_Kaufmann, prob = 0.95)
...
# frame.
conodontPositions_Kaufmann <- DevonianPredict_Kaufmann$HDI
# Plot.
...
HDImidpoint_Kaufmann <- data.frame(midpoint = data.frame(t(DevonianModelKaufmann$HDI))[, 2],
                          stratPosition = DevonianModel_Kaufmann$predictPositions)
