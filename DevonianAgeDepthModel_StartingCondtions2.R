...
DevonianPositions <- read.csv("DevonianPositions_KaufmannStd.csv", header = TRUE)
...
# Update stratigraphic position of repeated dates.
  ...
  # If the value in the DevonianData$newPositions column is NA, put the value 
  # from DevonianData$positions into the DevonianData$newPositions column.
  DevonianData$newPositions <- ifelse(is.na(DevonianData$newPositions), DevonianData$positions, DevonianData$newPositions)
  
  baseD = conodontPositions[which(conodontPositions$ids == "base of Devonian"), "0.5"]

# if newPositions is NA, put value from positions in newPositions
